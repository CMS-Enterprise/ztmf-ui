...
/**
 * Runtime configuration injected by config.js, a classic script served next
 * to the bundle (generated by `make frontend-env` locally, rendered from a
 * task-definition environment variable in a PR environment, an empty stub in
 * deployed dev/prod). Optional everywhere so the app works with no config.js.
 */
interface Window {
  ZTMF_RUNTIME_CONFIG?: {
    /** Bearer token enabling auth bypass where OIDC is absent. */
    authToken?: string
  }
}
