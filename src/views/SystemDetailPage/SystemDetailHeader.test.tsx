...
// system's questionnaire. The questionnaire route is keyed on :fismasystemid,
// the same key this page is routed by, so the link is a plain shareable anchor
// (ztmf-misc#386).
...
function renderHeader(props: Partial<typeof BASE_PROPS> = {}) {
  const router = createMemoryRouter(
    [
      ...
      {
        path: '/systems/:fismasystemid/questionnaire',
...
it('targets the questionnaire keyed on the fismasystemid', () => {
  renderHeader()
  // Same URL shape the dashboard's own questionnaire action builds
  // (FismaTable openQuestionnaire), so both entry points share one link.
  // Bare id with no trailing segments: the omitted datacall is what makes
  ...
  ).toBe('/systems/1002/questionnaire')
})

it('follows the anchor on a plain click and lands on this exact system', () => {
  const router = renderHeader()
  const link = screen.getByRole('link', { name: 'Questionnaire' })
  // jsdom has no navigation; drive the router to the href the anchor carries.
  act(() => {
    void router.navigate(link.getAttribute('href')!)
  })
  expect(router.state.location.pathname).toBe('/systems/1002/questionnaire')
})
